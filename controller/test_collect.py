...
RETRY = []
RETRY.append('cd SDN/host/ && sudo python3 test_run.py')
...
def recv_file(host_ips):
    ...
    for ip in host_ips:
        ...
        sftp = ssh.open_sftp()
        sftp.get(Config.BASE_PATH + "controller_" + ip + ".rules.enc", Config.HOST_BASE_PATH + "host_" + ip + ".rules.enc")
        sftp.close()
        ssh.close()
